...
def main():
    ...
    # print("html.keys(): ", html.keys(), "\n")
    
    problem_set = html["stat_status_pairs"]
    print(problem_set[10])

    # parse_proble_set(problem_set)

    d = []
    for i in range(len(problem_set)):
        title = problem_set[i]["stat"]["question__title_slug"]
        id = int(problem_set[i]["stat"]["frontend_question_id"])
        d.append((id, title))
    d.sort(key=lambda x: x[0])
    with open ("id2slug.txt", 'w') as f:
        for id, title in d:
            f.write(str(id) + "\t" + str(title) + "\n")
